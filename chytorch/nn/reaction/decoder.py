...
from torch.nn import GELU, Module, ModuleList
...
class ReactionDecoder(Module):
    def __init__(self, max_neighbors: int = 14, max_distance: int = 10, d_model: int = 1024, n_in_head: int = 16,
        ...
        if shared_layers:
            if shared_ex_weights:
                # attach encoder attrs to decoder
                encoder = self.molecule_encoder.layer
                layer = DecoderLayer.__new__(DecoderLayer)
                super(Module, layer).__init__()
                layer.mha_attn = encoder.self_attn
                layer.linear1 = encoder.linear1
                layer.linear2 = encoder.linear2
                layer.norm1 = encoder.norm1
                layer.norm2 = encoder.norm2
                layer.dropout1 = encoder.dropout1
                layer.dropout2 = encoder.dropout2
                layer.dropout3 = encoder.dropout3
                layer.activation = encoder.activation
                self.layers = [layer] * num_ex_layers
            else:
                self.layers = layers = []
                for encoder in self.molecule_encoder.layers:
                    layer = DecoderLayer.__new__(DecoderLayer)
                    super(Module, layer).__init__()
                    layer.mha_attn = encoder.self_attn
                    layer.linear1 = encoder.linear1
    ...
    def forward(self, batch: Tuple[TensorType['batch*2', 'atoms', int],
        ...
        atoms, neighbors, distances, d_mask = batch

        n = atoms.size(1)
        # BxN > Bx1x1xN > BxHxNxN > B*HxNxN
        d_mask = d_mask.view(-1, 1, 1, n).expand(-1, self.nhead, n, -1).flatten(end_dim=1)
        ...
        if averaged_weights:  # average attention weights from each layer
            ...
            for lr in self.layers[:-1]:  # noqa
                x, a = lr(x, rct, d_mask, need_weights=True)
                w.append(a)
            x, a = self.layers[-1](x, rct, d_mask, need_embedding=need_embedding, need_weights=True)
            w.append(a)
        ...
        for lr in self.layers[:-1]:  # noqa
            x, _ = lr(x, rct, d_mask)
        x, a = self.layers[-1](x, rct, d_mask, need_embedding=need_embedding, need_weights=need_weights)
        if need_embedding:
            if need_weights:
                return x, a
            return x
